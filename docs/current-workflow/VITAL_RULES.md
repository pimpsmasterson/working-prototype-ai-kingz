...
- Important: In automated start/cleanup scripts, do NOT run `pm2 kill`, `pm2 delete all`, or remove the `.pm2` directory globally — these actions will destroy saved process lists and break persistent daemons. Prefer targeted restarts (e.g. `pm2 delete vastai-proxy` then `pm2 start ... --update-env`) and run `pm2 save` afterward.

- To ensure PM2-managed apps survive reboots, run:

  ```powershell
  pm2 startup -u <your-user> --hp <home-path>
  pm2 save
  ```
...
- Resilience: On Windows, the one-click start will attempt to create a scheduled task `AIKings_PM2_Watchdog` which runs every 5 minutes to ensure `pm2` and `vastai-proxy` are online and will auto-heal by restarting them. If creation fails, run the following as admin to create the task manually:

```powershell
$taskName = 'AIKings_PM2_Watchdog'
$script = 'C:\path\to\repo\scripts\powershell\ensure_pm2.ps1'
schtasks /Create /TN $taskName /SC MINUTE /MO 5 /TR "powershell -NoProfile -ExecutionPolicy Bypass -File `"$script`"" /RL HIGHEST /F
```

- IMPORTANT: The watchdog runs in a limited loop and tries only safe restarts. For full system-level guarantees, ensure PM2 startup is configured (`pm2 startup`, `pm2 save`) and consider a Windows service wrapper for PM2 if you have strict uptime requirements.
