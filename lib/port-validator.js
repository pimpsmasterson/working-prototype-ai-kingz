...
async function validateInstancePorts(instance) {
  ...
  // Priority 4: Hardcoded port 18188 for Vast.ai instances (common external mapping)
  portCandidates.push({ port: 18188, source: 'vastai-standard-mapping' });
...
/**
 * Validate a Cloudflare tunnel URL for ComfyUI access.
 * This is the preferred method for accessing ComfyUI on Vast.ai instances
 * as it bypasses NAT, firewall, and port forwarding issues.
 * 
 * @param {string} tunnelUrl - The tunnel URL (e.g., https://abc123.trycloudflare.com)
 * @param {number} timeoutMs - Request timeout in milliseconds
 * @returns {Promise<{success: boolean, data?: object, error?: string}>}
 */
async function validateTunnelUrl(tunnelUrl, timeoutMs = 15000) {
  if (!tunnelUrl) {
  ...
  if (!tunnelPattern.test(tunnelUrl)) {
    return {
      success: false,
      error: `Invalid tunnel URL format: ${tunnelUrl}`,
      expected: 'https://<subdomain>.trycloudflare.com'
    };
  }

  console.log(`[PortValidator] Validating tunnel URL: ${tunnelUrl}`);

  try {
    const systemStatsUrl = `${tunnelUrl}/system_stats`;
    const result = await testHTTPEndpoint(systemStatsUrl, timeoutMs);

    if (result.success) {
      console.log(`[PortValidator] ✅ Tunnel URL validated: ${tunnelUrl}`);
      return {
        success: true,
        tunnelUrl,
        data: result.data,
      ...
      console.log(`[PortValidator] ⚠️ Tunnel validation failed: ${result.error}`);
      return {
        success: false,
        tunnelUrl,
        error: result.error
    ...
    console.log(`[PortValidator] ❌ Tunnel validation error: ${err.message}`);
    return {
      success: false,
      tunnelUrl,
      error: err.message
...
/**
 * Validate instance connectivity, preferring tunnel URL over direct port access.
 * This is the main validation function to use when checking if an instance is ready.
 * 
 * @param {object} instance - Instance object with optional tunnelUrl and port info
 * @param {string} instance.tunnelUrl - Cloudflare tunnel URL (preferred)
 * @param {string} instance.publicIp - Public IP for direct port access (fallback)
 * @param {number} instance.port - Port number (default: 8188)
 * @returns {Promise<{success: boolean, connectionUrl?: string, method?: string, error?: string}>}
 */
async function validateInstanceWithTunnel(instance) {
  const { tunnelUrl, publicIp, port = 8188 } = instance || {};

  // Priority 1: Try tunnel URL first (most reliable)
  if (tunnelUrl) {
    console.log('[PortValidator] Trying tunnel URL first (preferred method)');
    const tunnelResult = await validateTunnelUrl(tunnelUrl);
    ...
    console.log('[PortValidator] Tunnel validation failed, falling back to direct port');
  }

  // Priority 2: Fall back to direct port access
  if (publicIp) {
    console.log('[PortValidator] Trying direct port access (fallback method)');
    const portResult = await validateInstancePorts(publicIp, port);

    if (portResult.success) {
      return {
        success: true,
        connectionUrl: portResult.connectionUrl,
        method: 'direct_port',
        details: portResult
      };
    }
    ...
    return {
      success: false,
      error: 'Both tunnel and direct port validation failed',
      tunnelError: tunnelUrl ? 'Tunnel not reachable' : 'No tunnel URL',
  ...
  return {
    success: false,
    error: 'No tunnel URL or public IP available for validation'
  };
}
