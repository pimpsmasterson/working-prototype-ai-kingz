# AI Kings Automated Startup Script
...
try {
    ...
    if ($health.StatusCode -eq 200) {
        ...
        if ($prewarmResult) {
            ...
            # Explain the status (ASCII-safe messages)
            if ($prewarmResult.status -eq 'searching') {
                Write-Host "`nInstance rental started - searching for GPU offers..." -ForegroundColor Green
            } elseif ($prewarmResult.status -eq 'already_present') {
                Write-Host "`nWarm instance already running" -ForegroundColor Green
            } elseif ($prewarmResult.status -eq 'already_prewarming') {
                Write-Host "`nAnother prewarm operation is in progress" -ForegroundColor Yellow
            } else {
                Write-Host "`nStatus: $($prewarmResult.status)" -ForegroundColor Cyan
            }

            Write-Host "`nCheck status with:" -ForegroundColor Cyan
            $maskedAdmin = Mask-Secret $adminKey
            Write-Host "Invoke-RestMethod -Uri 'http://localhost:3000/api/proxy/admin/warm-pool/status' -Headers @{ 'x-admin-key'='$maskedAdmin' }"
        } else {
